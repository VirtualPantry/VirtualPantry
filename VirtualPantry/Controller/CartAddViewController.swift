...
class CartAddViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    func sendDataToFirebase(_ sender: Any) {
        ...
        if error == nil {
            ...
            ref = db.collection("groceryItems").addDocument(data:[
            ...
            db.collection("groceryItems").document(ref!.documentID).updateData(["picPath" : picPath])
            
            addItems.append(name!)
            print(addItems)
            db.collection("users").document(uid).updateData(["groceryItems" : FieldValue.arrayUnion(addItems)]){ error in
